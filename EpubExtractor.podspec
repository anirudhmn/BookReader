#
# Be sure to run `pod lib lint EpubExtractor.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'EpubExtractor'
  s.version          = '0.2.1'
  s.summary          = 'iOS library to extract epubs'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
This library offers a light, efficent and easy to use interface to extract and parse epubs 2 or 3 
                       DESC

  s.homepage         = 'https://github.com/f-meloni/EpubExtractor'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'f-meloni' => 'franco.meloni91@gmail.com' }
  s.source           = { :git => 'https://github.com/f-meloni/EpubExtractor.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '9.3'
  s.osx.deployment_target  = '10.10'

  s.source_files = 'EpubExtractor/Classes/**/*'
  
  s.dependency 'SSZipArchive', '~> 2.1'
  s.dependency 'AEXML', '~> 4.3'
  s.swift_version = '4.2'
end
